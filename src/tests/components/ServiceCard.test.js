...
describe('<ServiceCard/> Component. A UI Card for individual app service', () => {
    ...
    it('given no `title` prop, renders `EasyVtu` as title', () => {
        expect(render(<ServiceCard/>).getByTestId('service-title').children[0]).toBe('EasyVtu')
    });
    ...
    it('given resource id of an image via `source` prop, loads that image as service icon', () => {
        ...
        const {getByTestId} = render(<ServiceCard iconSrc={resourceId}/>);
