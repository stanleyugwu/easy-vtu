...
const formFieldsValidator = (values) => {
  ...
  if (values.referrer && values.referrer.trim().length > 10)
    errors.referrer = "Invalid Referrer ID";
...
const SignUpScreen = ({ navigation }: RootStackScreenProps<"Sign-Up">) => {
  ...
  const phoneInputRef = React.useRef<PhoneInput<typeof TextInput>>();
  const phoneNumberCache = React.useRef<string>();
  ...
  return (
    <ScrollView style={tw`p-0`}>
      <View
        style={tw.style(
          "mx-auto w-full h-full px-4 bg-transparent pb-2 mt-2"
        )}
      >
        {formStep == 2 ? (
          <BackButton
            buttonText="STEP 1"
            textStyle={tw`font-sans-semibold`}
