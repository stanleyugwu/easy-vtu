...
import { ImageBackground, View, Image, ActivityIndicator, StatusBar, Modal, BackHandler  } from 'react-native';
...
import tileBg from '../assets/images/tile_signup.png';
...
const SignInScreen = (props) => {
    ...
    const goTosignIn = () => { props.navigation.navigate('Sign-In'); };

    const goToSignUp = React.useCallback(() => { props.navigation.navigate('Sign-Up'); },[]);

    const goToForgotPassword = React.useCallback(() => { props.navigation.navigate('Forgot-Password') },[])
    ...
    //eye slash icon
    const eyeIcon = React.useMemo(() => (<Icon onPress={() => setPasswordVisible(!passwordVisible)} name={passwordVisible ? "eye-off-sharp" : "eye-sharp"} size={22} color={tw.color('primary')}/>), [passwordVisible])

    return (
        <SafeArea containerStyle={tw`p-0 h-full`}>
            ...
            >
                <View style={tw`mx-auto w-full h-full px-4 mt-5`}>
                    <Image
                        source={appIcon}
                        style={{ width: 100, height: 100, resizeMode: 'contain', alignSelf: 'center',flexGrow:0 }}
                    />
                    <Title>Welcome Back!</Title>
                    <Divider style={tw`h-1 bg-primary rounded-full w-20 mx-auto mt-1 mb-6`}/>
                    ...
                    <View>
                        ...
                        />
                        {emailError ? (<Text style={tw`text-red-700 pl-2 text-sm text-left font-sans`}>&gt; {emailError}</Text>) : null}
                    </View>
                    ...
                    <View style={tw`mt-4`}>
                        ...
                        />
                        {passwordError ? (<Text style={tw`text-red-700 pl-1 text-sm text-left font-sans`}>&gt; {passwordError}</Text>) : null}
                    </View>
                    
                    <Text style={tw`underline text-primary my-4 font-sans-semibold`} onPress={goToForgotPassword}>
                        FORGOT PASSWORD?
                    </Text>
                    ...
                    />
                        <Text style={tw`mt-4`}>
                            Not a member yet? {" "}
                            <Text style={tw`underline text-primary font-sans-semibold`} onPress={goToSignUp}>
                                SIGN UP
                            </Text>
                        </Text>
                </View>
